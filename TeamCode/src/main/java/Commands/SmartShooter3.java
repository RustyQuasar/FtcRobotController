...
public class SmartShooter3 {
    ...
    public void aim(boolean autoAim) {
        ...
        if (Vision.hasTarget()) {
            double neckDeltaHeading = neckHeading - botHeading;
        ...
        double time = Math.sqrt(distance / 39.37);
        double[] botWillTravel = new double[]{Constants.OdometryConstants.fieldVels.linearVel.x * time, Constants.OdometryConstants.fieldVels.linearVel.y * time};
        xChange += botWillTravel[0];
        yChange += botWillTravel[1];
        distance = Math.sqrt(Math.pow(xChange, 2) + Math.pow(yChange, 2));
    ...
    public boolean transfer(boolean buttonPressed) {
    ...
    private double xTurn(double angleToTurnDeg) {
        double angleDiff = angleToTurnDeg;
        if (angleDiff < 0){
            angleDiff += 360;
        }
